...
(defn bar_charts
  ...
  [[:h3 "Popular Programming Language Paradigms"]
   [:ol
    ...
    [:li "Load the " (c core) ", " (c charts) ", and " (c io) " libraries; this will enable us to load our data:"
...
(def pl_data_dump (read-dataset \"http://bit.ly/cR4sNd\" :header true :delim \\tab))")
    ...
    [:li "Because Freebase is imperfect in how it extracts information, many languages do not have a paradigm, so we want to make sure to remove these:"
         (code "(def paradigms_grouped (sel pl_data_dump :cols :language_paradigms
                                         :filter #(not (= \"\" (first %)))))")
         "From the remaining languages, using " (c "(col-names pl_data_dump)") "shows us that some languages (like Clojure) have multiple paradigms."
         " For those languages that have multiple paradigms, paradigms are comma separated in the column."
         " Clojure's paradigm list reads \"Functional programming,Multi-paradigm programming language,Concurrent computing\"."]
    [:li "Use " (c map) " and " (c reduce) " to turn those languages with multiple paradigms into a dataset of individual paradigms:"
...
(defn b_and_w_plots
  ...
  [[:h3 "State Population Box and Whisker Plots"]
   [:ol
    [:li "Import the following data:"
         (code "(def pop_data_dump (read-dataset \"http://bit.ly/cqSGMz\" :header true :delim \\,))")
         "The " [:a {:href "http://www.census.gov/"} "U.S. Census Bureau"] " posts " [:a {:href "http://www.census.gov/popest/datasets.html"} "population datasets"] " online that are freely available for download."
         " The dataset we just loaded contains population information from 2000 through 2009, including population change, births, and deaths."
    ...
    [:li "We'll use a box plot to display the population in each region:"
         (code "(with-data (sel pop_data :except-rows [0 1 2 3 4 56])
           (def pop_plot (box-plot :POPESTIMATE2009
                                   ...
                                   :legend true))
           (view pop_plot))")
         "The \"REGION\" column indicates which of the four US regions the state is in and \"DIVISION\" further breaks down the regions."
